...
an nn.Module whose forward(x:[N,in_channels,512,512]) -> logits [N,1,512,512] (default 3;
4 for RGBD), which is the only contract the training loop and winmol_unet.export.export_to_onnx
require.

encoder_weights defaults to None (no ImageNet download -> hermetic/offline); pass
'imagenet' for a pretrained encoder (better accuracy, needs network). smp adapts pretrained
first-conv weights automatically when in_channels != 3 (encoder_weights="imagenet" still works).
Non-UNet models are ONNX-only: the Keras HDF5/.keras mirror is UNet-specific and is skipped for them.
"""
