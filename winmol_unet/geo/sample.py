...
def _load_geoms(path, dst_crs, species=None, label="layer", quiet=False, amodal=False):
    ...
    With amodal=True, stem fragments sharing an `id` are bridged across their
    occlusion gaps first (scripts/amodal_stems.py), so the masks carry the whole
    ...
    if amodal:
        # scripts/amodal_stems.py has not (yet) moved into the package with the rest
        # of the geo code; import it by path so amodal stays opt-in and lazy
