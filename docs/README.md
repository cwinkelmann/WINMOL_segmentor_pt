...
| [`unet_vs_ronneberger.md`](unet_vs_ronneberger.md) | Our UNet vs the paper and vs the R port, including the zero-gradient `k_round` loss. | **current** |
...
Read the correction banner at the top of each before using any number from it.

| document | what is still good | what is void |
|---|---|---|
| [`synthetic-pretraining-beech.md`](synthetic-pretraining-beech.md) | The synthetic-pretraining measurements themselves. | Its explanation of why a Bachsee holdout gives F1 0.0 (colour domain / label doubt). |
| [`preprocessing-comparison.md`](preprocessing-comparison.md) | The fixed-metre arms' relative ordering. | **Arm C (native + multiscale)** — Bachsee_north was the validation site, so checkpoint selection ran on a signal pinned near zero. |
| [`session-report.md`](session-report.md) | The narrative of what was run. | Same two items as above. |
...
`superpowers/specs/` — export, augmentation, single-stage training, wandb, CPU inference,
preprocessing comparison, [scale augmentation](superpowers/specs/2026-08-13-scale-augmentation-design.md),
[Tegel R12/R13](superpowers/specs/2026-08-16-tegel-r12-r13-design.md).
